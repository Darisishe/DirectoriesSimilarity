...
DirectoriesComparator::DirectoriesComparator(const std::string& dir1, const std::string& dir2, double percentage, const IStringDistance& metric)
  : percentage_(percentage),
    ...
    directory2_(readDirectory(dir2)) {
}
...
SimilarityInfo DirectoriesComparator::calculateSimilarities()  {
  ...
  for (int i = 0; i < directory1_.size(); ++i) {
    for (int j = 0; j < directory2_.size(); ++j) {
      ...
      double similarity = metric_.similarity(file1.getContent(), file2.getContent());
      if (similarity >= percentage_) {
        is_unique_dir1[i] = false;
        is_unique_dir2[j] = false;

        info.similar.emplace_back(file1.getName(), file2.getName(), similarity);
      }
    }
  }
